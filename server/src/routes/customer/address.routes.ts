...
customerAddressRouter.get('/addresses',
    asyncHandler(async (req: Request, res: Response) => {
        ...
        const items = [...addresses].sort((a, b) => Number(b.isDefault) - Number(a.isDefault)).map(mapAddress);
        res.json(ok({items}));
    })
)

customerAddressRouter.post('/addresses',
    asyncHandler(async (req: Request, res: Response) => {
        const dbUser = await getDbUserFromReq(req);
        ...
        const shouldMarkAsDefault = req.body.isDefault === true || address.length === 0;
...
customerAddressRouter.patch('/addresses/:addressId',
    asyncHandler(async (req: Request, res: Response) => {
        const dbUser = await getDbUserFromReq(req);
        ...
        const getAddressTheUserWantToEdit = 
        addresses.find(currentAddress => String(currentAddress._id) === addressId);
        ...
        getAddressTheUserWantToEdit.isDefault = shouldMarkAsDefault;
        ...
        const items = [...(foundUser.addresses) as AddressItem[]].sort((a, b) => Number(b.isDefault) - Number(a.isDefault)).map(mapAddress);
        res.json(ok({items}));
    })
)

customerAddressRouter.delete('/addresses/:addressId',
    asyncHandler(async (req: Request, res: Response) => {
        const dbUser = await getDbUserFromReq(req);
        const addressId = String(req.params.addressId || "").trim();
