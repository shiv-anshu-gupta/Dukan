...
type ProductSort = "recent" | "price-asc" | "price-desc" | "price-low" | "price-high";
...
customerProductRouter.get('/categories', asyncHandler(async (_req: Request, res: Response) => {
    const categories = await Category.find({}).sort({name : 1});
    res.json(ok(categories));
}));


customerProductRouter.get('/products', asyncHandler(async (req: Request<{}, {}, {}, ProductAppliedFilterListQuery>, res: Response) => {
...
customerProductRouter.get('/products/:id', asyncHandler(async (req: Request, res: Response) => {
    ...
    const relatedProducts = await Product.find({
        ...
        status : 'active',
    }).populate('category','name').sort({createdAt : -1}).limit(4);
