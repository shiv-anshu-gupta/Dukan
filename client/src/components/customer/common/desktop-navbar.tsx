...
import { Heart, LogInIcon, LogOut,  ShoppingBag,  ShoppingCart, Store, User, type LucideIcon } from "lucide-react";
...
function NavTextLink({
    ...
    return <Link to={href} className={textLink}>
...
export function CustomerNavbar() {
  ...
  return <header className={headerClass}>
    <div className={shell}>
        ...
        <span className={brandTitle}>Dukaan</span>
        </Link>
...

            {
                ...
                <DropdownMenu>
                    ...
                    >
                        ...
                        <DropdownMenuItem
                        onClick={()=> signOut()} className={dropdownItemLink}>
                            <LogOut className="h-4 w-4"/>
                            <span>Logout</span>
                        </DropdownMenuItem>
                    </DropdownMenuContent>
                </DropdownMenu>
                : (<NavTextLink href="/sign-in" label="Login" icon={LogInIcon}/>
            )}

            <Link to={"/cart"} className={iconLink}>
                <ShoppingCart className="h-4.5 w-4.5"/>
                <span className={cartBadge}>Cart</span>
            </Link>
        </nav>
